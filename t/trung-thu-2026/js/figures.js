// People: children, parents, grandparents, artisans, and the moon's Cuội and Hằng Nga, from one builder.
// Stylised (round heads, soft proportions) but jointed: each figure has a body mesh and hinged arms and legs,
// so it can walk, sit on a mat, beat a drum or hold up a lantern. Parts are vertex-coloured and merged per limb.
import * as THREE from 'three';
import { mergeGeometries } from 'three/addons/utils/BufferGeometryUtils.js';
import { GEO, lanternMat, pickColor, singleLantern } from './lanterns.js';
...
// A child carrying a lantern on a bamboo stick (star, carp or round). tick(t, speed) walks and sways it.
const SHIRTS = [0xe8453c, 0xf2c14e, 0x3a8fd8, 0xffffff, 0x6fcf97, 0xe56aa8, 0xf08a3c, 0x8a6ad8];
const BOTTOMS = [0x2a3550, 0x3b2a20, 0x1f3a5e, 0x5a2a3a, 0x3a5a8a];
export function makeKid(rand, kind = 'star', extra = {}) {
  const girl = rand() < 0.5;
  const p = makePerson({ age: 'kid', girl, top: SHIRTS[Math.floor(rand() * SHIRTS.length)], bottom: BOTTOMS[Math.floor(rand() * BOTTOMS.length)],
    skin: SKIN[Math.floor(rand() * 3)], hair: girl ? (rand() < 0.5 ? 'bob' : 'bun') : 'short', ...extra });
  let lan = null;
  if (kind) {
    // right arm raised forward holding the stick; the lantern hangs from its tip on a short string
    p.holding = true;
    p.pose('R', -1.1, 0.1, -0.5);
    const hand = p.arms[1].el;
    const stickLen = 0.75;
    const stickM = new THREE.Mesh(new THREE.CylinderGeometry(0.006, 0.007, stickLen, 5).translate(0, stickLen / 2, 0), new THREE.MeshStandardMaterial({ color: 0xc8a060, roughness: 0.7 }));
    stickM.position.copy(p.arms[1].hand);
    stickM.rotation.x = 1.25;
    hand.add(stickM);
    const hang = new THREE.Group();
    hang.position.set(0, stickLen, 0);
    stickM.add(hang);
    const c = pickColor(kind, rand());
    lan = singleLantern(kind, c);
    lan.scale.setScalar(kind === 'star' ? 0.85 : 0.8);
    lan.position.y = kind === 'star' ? -0.2 : -0.14;
    if (kind === 'carp') lan.rotation.y = Math.PI / 2;
    const halo = new THREE.Sprite(new THREE.SpriteMaterial({ map: glowTex, color: c.clone().lerp(new THREE.Color(0xffc080), 0.55), blending: THREE.AdditiveBlending, depthWrite: false, opacity: 0.45 }));
    halo.scale.setScalar(0.7); halo.position.y = lan.position.y;
    hang.add(lan, halo);
    p.lantern = { hang, stick: stickM, color: c };
  }
  const ph = rand() * 6;
  p.g.scale.setScalar(0.9 + rand() * 0.22);
  p.tick = (t, speed = 1) => {
    const k = Math.min(1, speed);
    p.walk(t * 7 * Math.max(k, 0.001) + ph, k);
    if (p.lantern) {
      // keep the hanging lantern upright whatever the arm does, with a sway
      const h = p.lantern.hang;
      h.quaternion.copy(h.parent.getWorldQuaternion(new THREE.Quaternion())).invert();
      h.quaternion.multiply(new THREE.Quaternion().setFromEuler(new THREE.Euler(Math.sin(t * 1.7 + ph) * 0.12, p.g.rotation.y + (kind === 'carp' ? Math.PI / 2 : 0), Math.sin(t * 2.1 + ph) * 0.15 + Math.sin(t * 7 + ph) * 0.08 * k)));
    }
  };
  return p;
}
export { GEO as LANTERN_GEO, lanternMat };
